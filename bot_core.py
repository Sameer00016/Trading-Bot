...
def calculate_indicators(df: pd.DataFrame) -> pd.DataFrame:
    # Drop old indicator columns to avoid duplicates
    drop_cols = ["EMA_FAST", "EMA_SLOW", "RSI", "MACD", "MACD_SIGNAL", "MACD_HIST",
                 "BB_MID", "BB_UPPER", "BB_LOWER", "BB_PB", "PRICE_SLOPE"]
    ...
    df["BB_PB"] = bbands["BBB_20_2.0"]

    # Calculus slope (price trend derivative)
...
def backtest_strategy(df: pd.DataFrame) -> Dict[str, Any]:
    df["Signal"] = df.apply(lambda row: generate_signal(df), axis=1)
